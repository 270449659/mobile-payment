<template>
<div  class="product-attr-modal">
	<div class="main">
		<div class="header flex">
			<div>
				<img alt="商品图" :src="productAttrData.goods_image.large" class="avt" id="popupImg" ptag="7001.1.239" />
				<div v-if="productAttrData.current_price_state" class="price" id="priceSale2"><span class="price_decimals">¥</span><em>{{productAttrData.goods_price}}</em></div>
				<div v-else class="price" id="priceSale2"><span class="price_decimals">¥</span><em>{{productAttrData.goods_price[0]}}</em><span class="price_decimals">.{{productAttrData.goods_price[1]}}</span></div>
				<div class="stock" id="stock"><span></span></div>
				<div v-if="attrName.length" class="prop" id="popupSkuChoose">
				...
				<div v-else class="prop" id="popupSkuChoose"><span  class="prop-span">已选</span><span>{{curGoodsNum}}件</span></div> 
			</div>
			<div  @click="onClose" class="close" id="popupClose">X</div>
		</div><!-- header end -->
		<div class="body">
			<!-- 规格与颜色 -->
			<div v-if="productAttrData.goods_properties" class="popupSkuArea" id="popupSkuArea" >
				<div v-for="(item,index) in productAttrData.goods_properties" :key="index">	
					<div class="specifications-title">{{item.name}}</div>
					<div class="specifications-info flex">
					<div v-for="(items,indexs) in item.attrs" :key="indexs" :data-i="index"  :data-type="items.is_multiselect ? 'c' : 'r'" :data-id="items.id"  @click="choise_ver($event,items)"  :id="items.id" class="specifications-list">
					  <span class="item" :class="items.checked ? 'active' : ''" >    {{items.attr_name}}</span>
					</div>
					</div>
					<!-- <div class="colour-title">颜色</div>
					<div class="colour" id="skuPop2" >
			...
			<!-- 数量-->
			<div class="count-choose flex" id="popupCount">
				...
				<div class="num-wrap flex">
					...
					<div class="text-wrap"><input  type="number" :value="curGoodsNum" @input="changeGoodsNum($event)" data-type="input"  class="text"   id="buyNum1" /></div>
		...
		<div class="btns show">		            		            
			<div @click="goPurchase" class="btn yellow" id="buyBtn1">确认</div>
		</div>
	</div><!-- main end -->	
